...
const REFRESH_TOKEN_SECRET = process.env.REFRESH_TOKEN_SECRET || 'change_me_too';
...
async function createRefreshToken(userId: number) {
  const token = sign({ sub: userId, type: 'refresh' }, REFRESH_TOKEN_SECRET as Secret, { expiresIn: REFRESH_TOKEN_EXPIRES } as SignOptions);
...
export async function refreshTokens(refreshToken: string) {
  ...
  try {
    const payload = verify(refreshToken, REFRESH_TOKEN_SECRET as Secret) as unknown as { sub: number; type: string };
